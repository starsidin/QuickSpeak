...
from config import (
    ...
    SAVE_FOLDER, HOTKEY_ENABLED, HOTKEY_KEY, save_settings
)
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        super().__init__()
        self.setWindowTitle("WriteVoiceDown")
    ...
    def _start_hotkey_listener(self):
        ...
        def on_press(key):
            if key == target_key and not self._hotkey_pressed:
                ...
                QTimer.singleShot(0, self._hotkey_start_record)

        def on_release(key):
            if key == target_key and self._hotkey_pressed:
                self._hotkey_pressed = False
                QTimer.singleShot(0, self._hotkey_stop_record)
